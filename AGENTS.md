...
- Foundation governance: read `governance/AGENTS.md` and `governance/codex-access-contract.md` before project work. Do not use Windows PowerShell as the project logic layer; source edits go through the mounted repo or direct SSH/SFTP, and repo commands run from `/home/openclaw/foundation-source` with a POSIX shell.
